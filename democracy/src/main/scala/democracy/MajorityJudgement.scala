...
object Grade:
  /** @return
    ...
    * even number of grades, any of the two grades that are just before or after the middle of the sequence are correct
    * median values.
    *
    * Grades can be compared by using their `ordinal` method.
    *
...
@main def runElectionDemo(): Unit =
  val candidates = Set(Candidate("Alice"), Candidate("Bob"), Candidate("Charlie"))
  val election = Election("Sample Election", candidates)

  val ballots = Seq(
    Ballot(Map(Candidate("Alice") -> Grade.Excellent, Candidate("Bob") -> Grade.Good, Candidate("Charlie") -> Grade.Passable)),
    Ballot(Map(Candidate("Alice") -> Grade.Good, Candidate("Bob") -> Grade.Passable, Candidate("Charlie") -> Grade.Bad)),
    Ballot(Map(Candidate("Alice") -> Grade.VeryGood, Candidate("Bob") -> Grade.Good, Candidate("Charlie") -> Grade.Mediocre))
  )

  val winner = election.elect(ballots)
  println(s"The winner of the election is: ${winner.name}")
